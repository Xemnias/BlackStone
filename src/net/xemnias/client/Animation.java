...
public class Animation implements Renderable {
	...
	public Animation(SpriteSheet frames, int x1, int y1, int x2, int y2, boolean horizontalScan, int duration, boolean autoUpdate) {
		...
		if (!horizontalScan) {
			for (int x=x1;x<=x2;x++) {
				for (int y=y1;y<=y2;y++) {
					addFrame(frames.getSpriteAt(x, y), duration);
				}
			}
		} else {
			for (int y=y1;y<=y2;y++) {
	...
	public void drawS(float x, float y, float depht)
	{
		drawS(x, y, Color.white, depht);
	}
	
	public void drawS(float x, float y, Color filter, float depht)
	{
		drawS(x, y, getWidth(), getHeight(), filter, depht);
	}
	
	public void drawS(float x, float y, float width, float height, Color filter, float depht)
	{
		if (frames.size() == 0) {
			return;
		}
		...
		Frame frame = (Frame) frames.get(currentFrame);
		frame.image.drawS(x,y,width,height, depht, filter);
	}
	/**
	...
	private class Frame {
		...
		public Frame(int duration, int x, int y) {
			this.image = spriteSheet.getSpriteAt(x, y);
			this.duration = duration;
			this.x = x;
			this.y = y;
		} 
